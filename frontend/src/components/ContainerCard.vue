...
const sourceLabel = (s) => ({ baseline: '基线', update: '更新', rollback: '回退前' }[s] || s)
    ...
    <div class="flex flex-col gap-3.5 p-4 flex-1">
      ...
      <div class="rounded-xl bg-slate-50 dark:bg-slate-800/60 px-3 py-2 flex flex-col gap-2">
        ...
        <div class="flex items-center gap-2 h-6 text-xs">
          ...
          <span v-else class="text-[11px] text-slate-400">已最新</span>
        </div>
      </div>
        ...
        <div class="mt-3 space-y-1.5 max-h-60 overflow-y-auto pr-0.5">
          <button v-for="v in rbInfo.versions" :key="v.digest"
            ...
            :class="v.is_current
              ...
              : rbPick === v.digest
                ? 'bg-brand-50 dark:bg-brand-500/10 ring-1 ring-brand-400'
